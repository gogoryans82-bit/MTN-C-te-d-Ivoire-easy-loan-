...
function applyAsExistingUser() {
    console.log('[applyAsExistingUser] isRegistered:', isUserRegistered(), 'statut:', S.registrationStatus);
    if (isUserRegistered()) {
    ...
    showToast('🔗 Reliez votre portefeuille MoMo — 60 secondes', 'info', 3500);
...
function startMoMoRegistration(opts) {
    ...
    if (hint) hint.textContent = 'Appuyez pour sélectionner';
    clearErr('regErr');
...
function selectAccountType(type) {
    ...
    var names = { basic: 'Compte Basic', standard: 'Compte Standard', premium: 'Compte Premium' };
    document.querySelectorAll('.account-type').forEach(function (el) {
...
function updateRequirementsLimits() {
    var tc = ACCOUNT_TYPES[S.accountType] || ACCOUNT_TYPES.basic;
...
async function showTerms() {
    try {
        ...
        if (pre) pre.textContent = 'Chargement…';
        var data = await apiCall('/api/terms');
        if (!data.ok) { if (pre) pre.textContent = 'Impossible de charger les Conditions.'; return; }
...
// ─── DÉMARRAGE ───
function boot() {
    console.log('🔧 Démarrage MTN MoMo Côte d\'Ivoire v7.5...');
    ...
    console.log('✅ MTN MoMo Côte d\'Ivoire v7.5 prêt');
}
